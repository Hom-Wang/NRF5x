...
typedef struct ps_module_param
{
    ...
    uint32_t               block_size;     /**< Desired block size for persistent memory storage, for example, if a module has a table with 10 entries, each entry is size 64 bytes,
                                            ...
                                            *   data.
                                            */
    uint32_t               block_count;    /** Number of blocks requested by the module, minimum values is 1. */
 ...
 */
uint32_t pstorage_clear(pstorage_handle_t * p_base_id, uint32_t size);
